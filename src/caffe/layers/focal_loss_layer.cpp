...
void FocalLossLayer<Ftype, Btype>::LayerSetUp(
...
template <typename Ftype, typename Btype>
void FocalLossLayer<Ftype, Btype>::Reshape(
    const vector<Blob*>& bottom, const vector<Blob*>& top) {
...
template <typename Ftype, typename Btype>
void FocalLossLayer<Ftype, Btype>::Forward_cpu(
    const vector<Blob*>& bottom, const vector<Blob*>& top) {
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      loss -= alpha_ * pow(1.0 - pt, gamma_) * log(std::max(pt, min_dtype<Ftype>()));
      ++count;
    }
  }
...
template <typename Ftype, typename Btype>
void FocalLossLayer<Ftype, Btype>::Backward_cpu(const vector<Blob*>& top,
      ...
      for (int j = 0; j < inner_num_; ++j) {
        ...
        if (has_ignore_label_ && label_value == ignore_label_) {
          ...
          for (int c = 0; c < bottom[0]->shape(softmax_axis_); ++c) {
              ...
              if(c == label_value){
                  focal_diff =  alpha_ * 
                      pow(1 - pt, gamma_) * (gamma_ * pt * log(std::max(pt, min_dtype<Btype>())) + pt - 1);
              }
              else{
                  focal_diff = alpha_ * 
                     (pow(1 - pt, gamma_ - 1) * (-gamma_ * log(std::max(pt, min_dtype<Btype>())) * pt * pc)
                       + pow(1 - pt, gamma_) * pc);
              }
...
INSTANTIATE_CLASS_FB(FocalLossLayer);
